...
# Complete Learning Format — Week 17: RAG Assistant
...
Week 17 solves the project problem behind **RAG Assistant**.
...
- The concept `Retrieve-then-generate: why generation without retrieval hallucinates` helps solve part of this gap.
- The concept `Chunked retrieval: finding the right text fragments before generating` helps solve part of this gap.
- The concept `Prompt engineering: grounding instructions, source citation requirements` helps solve part of this gap.
- The concept `Hallucination detection: what happens when no relevant chunks exist` helps solve part of this gap.
- The concept ``QAService` orchestration: retrieve → prompt → call LLM → return citations` helps solve part of this gap.
- The concept ``ai/prompts.py`: where prompt templates live` helps solve part of this gap.
...
- Ask: what is the input for **RAG Assistant**?
- Ask: what is the owner for **RAG Assistant**?
- Ask: what is the transformation for **RAG Assistant**?
- Ask: what is the proof for **RAG Assistant**?
...
|------|----------------|---------------------|
| Retrieve-then-generate | Retrieve-then-generate: why generation without retrieval hallucinates | This term names one job in the Week 17 milestone. |
| Chunked retrieval | Chunked retrieval: finding the right text fragments before generating | This term names one job in the Week 17 milestone. |
| Prompt engineering | Prompt engineering: grounding instructions, source citation requirements | This term names one job in the Week 17 milestone. |
| Hallucination detection | Hallucination detection: what happens when no relevant chunks exist | This term names one job in the Week 17 milestone. |
| QAService` orchestration | `QAService` orchestration: retrieve → prompt → call LLM → return citations | This term names one job in the Week 17 milestone. |
| ai/prompts.py | `ai/prompts.py`: where prompt templates live | This term names one job in the Week 17 milestone. |
...
### Concept 1: Retrieve-then-generate: why generation without retrieval hallucinates
...
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
...
### Concept 2: Chunked retrieval: finding the right text fragments before generating
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 3: Prompt engineering: grounding instructions, source citation requirements
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 4: Hallucination detection: what happens when no relevant chunks exist
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 5: `QAService` orchestration: retrieve → prompt → call LLM → return citations
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 6: `ai/prompts.py`: where prompt templates live
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 17, it supports the milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
- Line 1: `def build_prompt(question: str, chunks: list[str]) -> str:` — This names a reusable action and shows what information it receives.
- Line 2: `context = "\n\n".join(chunks)` — This stores a clear intermediate value for the next step.
- Line 3: `return f"Use only this context:\n{context}\n\nQuestion: {question}"` — This produces the result or performs the declared setup step.
...
Week 17 contributes by making **rag assistant** clear enough to trust.
...
- What problem does Week 17 solve?
...
- Explain RAG Assistant in simple words.
...
- The topic: RAG Assistant.
- The milestone: `researchops ask "What is attention?"` returns a cited answer based on retrieved paper chunks.
...
Next week is Week 18: **Docker and Environment Configuration**.
This week prepares you by giving ResearchOps a clearer piece of behavior before the next milestone: `docker-compose up` starts the API and worker. `curl http://localhost:8000/health` responds.
- Run validation.
