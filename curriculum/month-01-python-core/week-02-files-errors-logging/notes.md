...
Week 2 solves the project problem behind **Files, Paths, Exceptions, and Logging**.
...
- The concept ``pathlib.Path` API: `.exists()`, `.glob()`, `.suffix`, `.stat()`` helps solve part of this gap.
- The concept ``open()` and context managers (`with` statement)` helps solve part of this gap.
- The concept `Custom exception hierarchy (`class ScanError(ResearchOpsError)`)` helps solve part of this gap.
- The concept ``try / except / finally`` helps solve part of this gap.
- The concept ``logging` module: levels, formatters, handlers` helps solve part of this gap.
- The concept `Structured log output` helps solve part of this gap.
...
- Ask: what is the input for **Files, Paths, Exceptions, and Logging**?
- Ask: what is the owner for **Files, Paths, Exceptions, and Logging**?
- Ask: what is the transformation for **Files, Paths, Exceptions, and Logging**?
- Ask: what is the proof for **Files, Paths, Exceptions, and Logging**?
...
|------|----------------|---------------------|
| pathlib.Path` API | `pathlib.Path` API: `.exists()`, `.glob()`, `.suffix`, `.stat()` | This term names one job in the Week 2 milestone. |
| open()` and context managers (`with` statement) | `open()` and context managers (`with` statement) | This term names one job in the Week 2 milestone. |
| Custom exception hierarchy (`class ScanError(ResearchOpsError)`) | Custom exception hierarchy (`class ScanError(ResearchOpsError)`) | This term names one job in the Week 2 milestone. |
| try / except / finally | `try / except / finally` | This term names one job in the Week 2 milestone. |
| logging` module | `logging` module: levels, formatters, handlers | This term names one job in the Week 2 milestone. |
| Structured log output | Structured log output | This term names one job in the Week 2 milestone. |
...
### Concept 1: `pathlib.Path` API: `.exists()`, `.glob()`, `.suffix`, `.stat()`
...
- **ResearchOps use:** In Week 2, it supports the milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
...
### Concept 2: `open()` and context managers (`with` statement)
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 2, it supports the milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 3: Custom exception hierarchy (`class ScanError(ResearchOpsError)`)
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 2, it supports the milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 5: `logging` module: levels, formatters, handlers
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 2, it supports the milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
### Concept 6: Structured log output
- **Plain meaning:** This is a named tool for solving one part of the chapter problem.
- **Why it exists:** Real projects become confusing when this concern is unnamed.
- **ResearchOps use:** In Week 2, it supports the milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
- **Input question:** What data, command, file, request, or state reaches this concept?
...
try:
    ...
    logger.error("folder is missing: %s", folder)
    raise ScanError(str(error)) from error
```

Line-by-line explanation:
- Line 1: `try:` — This controls what happens when the normal path changes.
- Line 2: `pdfs = find_pdfs(folder)` — This stores a clear intermediate value for the next step.
- Line 3: `except FileNotFoundError as error:` — This controls what happens when the normal path changes.
- Line 4: `logger.error("folder is missing: %s", folder)` — This performs one small visible step in the workflow.
- Line 5: `raise ScanError(str(error)) from error` — This reports a meaningful failure instead of hiding it.
...
Week 2 contributes by making **files, paths, exceptions, and logging** clear enough to trust.
...
- What problem does Week 2 solve?
...
- Explain Files, Paths, Exceptions, and Logging in simple words.
...
- The topic: Files, Paths, Exceptions, and Logging.
- The milestone: PDF discovery uses `pathlib` throughout. Custom exceptions wrap file errors. Log messages include level, name, and message.
...
Next week is Week 3: **OOP, Dataclasses, and Domain Modeling**.
This week prepares you by giving ResearchOps a clearer piece of behavior before the next milestone: `Paper`, `ParsedDocument`, `IngestionResult`, `FailedDocument`, `SearchResult` are defined, type-annotated, and tested.
- Run validation.
