...
- **Lines 14** defines `ParsingError`. This is a narrower specialization. Anything that goes wrong during document parsing should raise `ParsingError` or one of its subclasses. Notice that `ParsingError` inherits from `ResearchOpsError`, not from `Exception` directly. That keeps the hierarchy connected.
